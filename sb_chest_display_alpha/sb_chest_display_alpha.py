...
class Plugin(PluginBase):
    def onInit(self, inputs=None):
        ...
        self.config.options('int', {
            'scda_text_display_size': 15,
            'scda_max_items_per_box': 20,
            'scda_width_edge_spacing': 15, # Default is 15
            'scda_text_y_spacing': 20,
            'scda_text_x_padding': 5,
            'scda_text_y_padding': 5,
        })
        self.config.options('color', {
            'scda_text_modified_label_colour': 0xff79f071,
            'scda_text_custom_label_colour': 0xff3d70f0,
            'scda_text_experimental_label_colour': 0xffd04ef0,
            'scda_text_prototype_label_colour': 0xfff0b03d,
            'scda_text_modified_colour': 0xff79f071,
            'scda_text_custom_colour': 0xff638cf3,
            'scda_text_experimental_colour': 0xffde83f4,
            'scda_text_prototype_colour': 0xfff6d291,
        })
        self.config.options('bool', {
            'scda_display_box': True,
        })
    ...
    def onPresent(self):
        ...
        if not self.draw or not self.config.scda_display_box:
            return
        ...
        for game_object in util.worldobjects(client_world.mySubWorld.asNativeSubWorld):
                if util.getClassName(game_object) == "Chest":
                    ...
                    box_width = get_longest_name_object.w + self.config.scda_width_edge_spacing
                    # (self.config.scda_text_y_spacing * height_scale) + (self.config.scda_text_y_padding * 2)
                    #get_longest_name_object.h * height_scale + (self.config.scda_text_y_padding * height_scale) + self.config.scda_text_y_padding
                    box_height = (self.config.scda_text_y_spacing * height_scale) + (self.config.scda_text_y_padding * 2) 
                    # Remove display if the user opens the chest
                    if chest.pos == -1:
                        
                        if (height_scale > self.config.scda_max_items_per_box):
                            next_box_space = 0
                            ...
                            box_height = (self.config.scda_text_y_spacing * self.config.scda_max_items_per_box) + (self.config.scda_text_y_padding * 2)
                            # Create dynamic display boxes
                            for i in range(math.ceil(height_scale / self.config.scda_max_items_per_box)):
                                if i == 0:
                                    next_box_space = (box_width) + (self.config.scda_text_x_padding)
                                    next_box_space = next_box_space * -1
                                else:
                                    next_box_space = (box_width * (i - 1)) + (self.config.scda_text_x_padding * (i - 1))
                            ...
                            for i in range(len(item_display_list_indexes)):
                                ...
                                for j in range(len(item_display_list_indexes[i])):
                                    ...
                                    loot["item"].draw((x - (box_width // 2) + next_box_space) + self.config.scda_text_x_padding, (y - box_height + (self.config.scda_text_y_spacing  * display_index)) + self.config.scda_text_y_padding)
                                    display_index += 1
                                    if (len(loot["boosts"]) != 0):
                                        for boost in loot["boosts"]:
                                            boost.draw((x - (box_width // 2) + next_box_space) + (self.config.scda_text_x_padding * 2), (y - box_height + (self.config.scda_text_y_spacing  * display_index)) + self.config.scda_text_y_padding)
                                            display_index += 1
                        else:
                            # If we only less than self.config.scda_max_items_per_box, only display one box
                            self.refs.XDL_FillRect(
                            x - (box_width // 2), y - box_height, box_width, box_height, 0xA0000000, lib.BLENDMODE_BLEND)
                            display_index = 0
                            for index, element in enumerate(self.display_dict[game_object.objId]):
                                element["item"].draw((x - (box_width // 2)) + self.config.scda_text_x_padding, (y - box_height + (self.config.scda_text_y_spacing  * display_index)) + self.config.scda_text_y_padding)
                                display_index += 1
                                if (len(element["boosts"]) != 0):
                                    for boost in element["boosts"]:
                                        boost.draw((x - (box_width // 2)) + (self.config.scda_text_x_padding * 2), (y - box_height + (self.config.scda_text_y_spacing  * display_index)) + self.config.scda_text_y_padding)
                                        display_index += 1
...
def addItemToLoggingDisplay(self, loot_item, name, boost_length, boosts):
    ...
    item.size = self.config.scda_text_display_size
    item.text = name
    if boost_length != -1:
        if boost_length == 1:
            label_colour = self.config.scda_text_modified_label_colour
            boost_colour = self.config.scda_text_modified_colour
        if boost_length == 2:
            label_colour = self.config.scda_text_custom_label_colour
            boost_colour = self.config.scda_text_custom_colour
        if boost_length == 3:
            label_colour = self.config.scda_text_experimental_label_colour
            boost_colour = self.config.scda_text_experimental_colour
        if boost_length == 4:
            label_colour = self.config.scda_text_prototype_label_colour
            boost_colour = self.config.scda_text_prototype_colour
        ...
        for boost in reFieldToList(boosts, 'struct StatBoost *'):
            ...
            boost_display.size = self.config.scda_text_display_size
